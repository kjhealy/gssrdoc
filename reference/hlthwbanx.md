# Looked up online - anxiety or stress

hlthwbanx

## Source

General Social Survey https://gss.norc.org

## Details

Question (During the past 12 months, how often, if at all, have you used
the internet to look for information on the following topics?)
Information related to anxiety, stress, or similar problems?

## Values

- `[1]` never

- `[2]` seldom

- `[3]` sometimes

- `[4]` often

- `[5]` very often

- `[NA(d)]` can't choose `[NA(i)]` iap `[NA(j)]` I don't have a job
  `[NA(m)]` dk, na, iap `[NA(n)]` no answer `[NA(p)]` not imputable
  `[NA(r)]` refused `[NA(s)]` skipped on web `[NA(u)]` uncodeable
  `[NA(x)]` not available in this release `[NA(y)]` not available in
  this year `[NA(z)]` see codebook

## Question Years and Ballots

|         |       |
|---------|-------|
| ballots | years |
| C       | 2022  |

## Overview

For further details see the [GSS Data Explorer page for
`hlthwbanx`](https://gssdataexplorer.norc.org/variables/7811/vshow).

Counts by year:

|       |                            |       |        |           |       |            |              |      |                |       |
|-------|----------------------------|-------|--------|-----------|-------|------------|--------------|------|----------------|-------|
| year  | not available in this year | never | seldom | sometimes | often | very often | can't choose | iap  | skipped on web | Total |
| 2022  | \-                         | 357   | 245    | 289       | 129   | 75         | 39           | 2406 | 4              | 3544  |
| Total | 0                          | 357   | 245    | 289       | 129   | 75         | 39           | 2406 | 4              | 3544  |
